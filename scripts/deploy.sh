...
# ── Configuration ────────────────────────────────────────────────────────────
PI5_HOST="pi@192.168.1.231"
PIZERO_HOST="pi@192.168.1.247"
APP_NAME="hairStyleBuddy"
REMOTE_SRC="/home/pi/${APP_NAME}"
REMOTE_BUILD="${REMOTE_SRC}/build"
DEPLOY_DIR="/home/pi/${APP_NAME}"
...
ssh "$PI5_HOST" bash << ENDSSH
    set -e
    ssh pi@192.168.1.247 "mkdir -p ${DEPLOY_DIR}"
    scp "${REMOTE_BUILD}/${APP_NAME}" "pi@192.168.1.247:${DEPLOY_DIR}/${APP_NAME}"
ENDSSH
